...
async function event(runId: string, status: AutomationStatus, message: string, detail?: unknown) {
...
async function update(runId: string, status: AutomationStatus, currentStep: string, values: { job?: JobDetails; questions?: AdapterQuestion[]; pause?: unknown; error?: string | null } = {}) {
...
export class AutomationManager {
  async create(userId: string, jobUrl: string, autoSubmit = false, testMode = false) {
  ...
  async get(userId: string, runId: string) {
    const row = await db.automationRuns.findOne({ id: runId, user_id: userId }) as RunRow | undefined
    if (!row) return null
    const events = await db.automationEvents.find({ run_id: runId }) as Array<{ status: string; message: string; detail_json: string | null; created_at: string }>
  ...
  async list(userId: string) { 
    const rows = await db.automationRuns.find({ user_id: userId }) as unknown as RunRow[]
  ...
  async pauseRun(userId: string, runId: string) {
    ...
    if (row.status.startsWith('PAUSED_')) return await this.get(userId, runId)
  ...
  private async process(runId: string, userId: string) {
    ...
    try {
      const row = await db.automationRuns.findOne({ id: runId, user_id: userId }) as RunRow
      const adapter = detectAdapter(row.job_url)
      if (!adapter) throw new Error('The job-board adapter is unavailable.')
      if (!active) {
        await update(runId, 'OPENING_JOB', 'LAUNCHING_BROWSER'); await event(runId, 'OPENING_JOB', 'Opening a visible Chrome window.')
        ...
        if (!persistent?.browser.isConnected()) {
          ...
          browser.on('disconnected', async () => {
            ...
            for (const [activeRunId, activeRun] of activeRuns) {
              ...
              const current = await db.automationRuns.findOne({ id: activeRunId }) as { status: AutomationStatus } | undefined
      ...
      if (stopForManualPause()) return
      await update(runId, 'EXTRACTING_JOB', 'WAITING_FOR_FORM'); await event(runId, 'EXTRACTING_JOB', `Waiting for the ${this.boardName(adapter.id)} application form.`)
      ...
      if (stopForManualPause()) return
      await update(runId, 'FILLING_APPLICATION', 'RESOLVING_QUESTIONS', { job, questions }); await event(runId, 'FILLING_APPLICATION', `Found ${questions.length} application fields.`, { questionCount: questions.length })

      const savedResume = await db.profiles.find({ user_id: userId }) as { resume_storage_name: string | null; resume_filename: string | null; resume_mime: string | null } | undefined
      const tailored = await db.resumeOptimizations.findOne({ user_id: userId, job_url: row.job_url, status: 'APPROVED', storage_name: { $ne: null } }) as { storage_name: string; job_title: string } | undefined
      ...
      const profileEducation = await db.profiles.find({ user_id: userId }) as { education_json: string } | undefined
      ...
      for (let questionIndex = 0; questionIndex < questions.length; questionIndex += 1) {
        ...
        if (question.inputType === 'file' && question.id === '_systemfield_resume') {
          ...
          if (question.required) return await this.pause(runId, question, 'A saved resume is required for this application.')
          continue
        }
        ...
        if (question.required) return await this.pause(runId, question, resolution.reason)
      ...
      const failedState = await db.automationRuns.findOne({ id: runId }) as { current_step: string; job_board: string } | undefined
      ...
      await update(runId, 'FAILED', 'FAILED', { error: message }); await event(runId, 'FAILED', message)
  ...
  private async pause(runId: string, question: AdapterQuestion, reason: string) {
    const pause = { question, reason, instruction: 'Complete this field in the visible browser, then click Continue automation.' }
    await update(runId, 'PAUSED_NEEDS_INPUT', 'WAITING_FOR_USER', { pause }); await event(runId, 'PAUSED_NEEDS_INPUT', `Waiting for “${question.text}”.`, pause)
  }

  private async pauseForBlocker(runId: string, status: 'PAUSED_LOGIN' | 'PAUSED_CAPTCHA', message: string, currentStep = 'WAITING_FOR_USER') {
    const pause = { reason: message, instruction: 'Complete the step in the visible browser, then click Continue automation.' }
    await update(runId, status, currentStep, { pause }); await event(runId, status, message)
  }
