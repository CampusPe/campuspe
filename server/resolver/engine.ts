...
async function loadCandidate(userId: string): Promise<CandidateContext> {
...
async function loadMemory(userId: string, normalized: string, canonical: string | null, job: JobContext) {
  const rows = await db.answerMemory.find({ user_id: userId }) as unknown as MemoryRow[]
...
export class AnswerResolver {
  ...
  async resolve(userId: string, question: FormQuestion, job: JobContext = {}, options: { testMode?: boolean } = {}): Promise<Resolution> {
    ...
    if (approvedDeclaration) return await this.finish(userId, question, job, normalized, canonical, { status: 'RESOLVED', answer: approvedDeclaration, source: 'L1_PROFILE', confidence: 1, requiresReview: true, canonicalField: canonical, explanation: 'Used your explicit approval for the no-recording and no-transcribing acknowledgement.' })
    ...
    if (manualReason && options.testMode && question.options?.length) {
      ...
      if (testAnswer) return await this.finish(userId, question, job, normalized, canonical, { status: 'RESOLVED', answer: testAnswer, source: 'L3_LLM', confidence: 1, requiresReview: true, canonicalField: canonical, explanation: 'Testing mode selected a visible option. This run cannot be submitted.' })
    }
    if (manualReason) return await this.finish(userId, question, job, normalized, canonical, { status: 'NEEDS_USER_INPUT', reason: manualReason, canonicalField: canonical, pauseCode: 'RESTRICTED_QUESTION' })
    ...
    if (l1 && question.options?.length) {
      ...
      if (!matchedOption) {
        if (canonical && (sensitiveFields.has(canonical) || nonInferableFields.has(canonical))) return await this.finish(userId, question, job, normalized, canonical, { status: 'NEEDS_USER_INPUT', reason: 'Your saved profile answer does not match any option shown by this employer.', canonicalField: canonical, pauseCode: 'MISSING_PROFILE_VALUE' })
        l1 = null
      } else l1 = { ...l1, answer: matchedOption }
    }
    if (l1) return await this.finish(userId, question, job, normalized, canonical, { status: 'RESOLVED', answer: l1.answer, source: 'L1_PROFILE', confidence: l1.confidence, requiresReview: l1.review, canonicalField: canonical, explanation: l1.explanation })
    if (canonical && sensitiveFields.has(canonical) && options.testMode) {
      const privateOption = nonDisclosureOption(question.options)
      if (privateOption) return await this.finish(userId, question, job, normalized, canonical, { status: 'RESOLVED', answer: privateOption, source: 'L3_LLM', confidence: 1, requiresReview: true, canonicalField: canonical, explanation: 'Testing mode used the employer’s non-disclosure option because no explicit demographic answer was saved.' })
    }
    if (canonical && sensitiveFields.has(canonical) && !options.testMode) return await this.finish(userId, question, job, normalized, canonical, { status: 'NEEDS_USER_INPUT', reason: 'This voluntary answer was not explicitly enabled and saved in the profile.', canonicalField: canonical, pauseCode: 'RESTRICTED_QUESTION' })
    ...
    if (memory) {
      ...
      if (matchedMemoryOption === null) return await this.finish(userId, question, job, normalized, canonical, { status: 'NEEDS_USER_INPUT', reason: 'Your previously approved answer does not match any option shown by this employer.', canonicalField: canonical, pauseCode: 'MISSING_PROFILE_VALUE' })
      await db.answerMemory.update({ id: memory.row.id }, { usage_count: ((memory.row as any).usage_count || 0) + 1, last_used_at: new Date().toISOString() })
    ...
    if (canonical && nonInferableFields.has(canonical) && !options.testMode) return await this.finish(userId, question, job, normalized, canonical, { status: 'NEEDS_USER_INPUT', reason: 'This factual or preference answer must come from your profile or a previously approved answer.', canonicalField: canonical, pauseCode: 'RESTRICTED_QUESTION' })
    if ((question.fieldType === 'select' || question.fieldType === 'boolean') && !question.options?.length) return await this.finish(userId, question, job, normalized, canonical, { status: 'NEEDS_USER_INPUT', reason: 'This is a choice control, but its visible options could not be read reliably. JobCopilot will not type a generated sentence into it.', canonicalField: canonical, pauseCode: 'MISSING_PROFILE_VALUE' })
    if (!this.llm && options.testMode) return await this.finish(userId, question, job, normalized, canonical, { status: 'RESOLVED', answer: this.testAnswer(question), source: 'L3_LLM', confidence: 1, requiresReview: true, canonicalField: canonical, explanation: 'Used a testing-only fallback because no model is configured. This run cannot be submitted.' })
    if (!this.llm) return await this.finish(userId, question, job, normalized, canonical, { status: 'NEEDS_USER_INPUT', reason: 'No reliable profile or approved-memory answer was found, and no LLM provider is configured.', canonicalField: canonical, pauseCode: 'LLM_NOT_CONFIGURED' })
    const draft = await this.llm.resolve({ question, normalizedQuestion: normalized, canonicalField: canonical, candidate, job })
    if (!draft && options.testMode) return await this.finish(userId, question, job, normalized, canonical, { status: 'RESOLVED', answer: this.testAnswer(question), source: 'L3_LLM', confidence: 1, requiresReview: true, canonicalField: canonical, explanation: 'Used a testing-only fallback because the model declined. This run cannot be submitted.' })
    if (!draft) return await this.finish(userId, question, job, normalized, canonical, { status: 'NEEDS_USER_INPUT', reason: 'The LLM declined to generate a reliable answer.', canonicalField: canonical, pauseCode: 'LLM_DECLINED' })
    if (draft.confidence < .75 && !options.testMode) return await this.finish(userId, question, job, normalized, canonical, { status: 'NEEDS_USER_INPUT', reason: 'The generated draft did not meet the confidence threshold.', canonicalField: canonical, pauseCode: 'LOW_CONFIDENCE' })
  ...
  async remember(userId: string, input: { question: FormQuestion; answer: AnswerValue; scope: 'global' | 'company'; company?: string }) {
  ...
  private async finish(userId: string, question: FormQuestion, job: JobContext, normalized: string, canonical: string | null, result: Resolution) {
