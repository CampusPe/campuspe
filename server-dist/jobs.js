...
const companyLabel = (value) => value.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/[-_]+/g, ' ').replace(/\b\w/g, (letter) => letter.toUpperCase());
const enumLabel = (value) => value.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/[-_]+/g, ' ');
const skillMatchers = [
...
function configuredCompanies(source) {
...
async function getJson(url) {
    ...
    if (!response.ok)
        throw new Error(`Job board returned HTTP ${response.status}`);
...
function inferWorkplace(location, remote, explicit) {
    if (typeof explicit === 'boolean')
        return explicit ? 'Remote' : 'On site';
    const value = asString(explicit);
    if (value && /^(remote|hybrid|onsite)$/i.test(value))
...
async function fetchLever(company) {
    ...
    return asArray(payload).map(asRecord).map((job) => { const categories = asRecord(job.categories); return normalize({ source: 'lever', company, id: job.id, title: job.text, location: categories.location, department: categories.team, remote: null, workplaceType: categories.workplaceType, employmentType: categories.commitment, publishedAt: job.createdAt, jobUrl: job.hostedUrl, applyUrl: job.applyUrl, description: `${asString(job.descriptionPlain) || ''} ${asString(job.additionalPlain) || ''}` }); }).filter((job) => Boolean(job));
}
...
const fetchers = { ashby: fetchAshby, greenhouse: fetchGreenhouse, lever: fetchLever, workable: fetchWorkable };
export async function getRecommendedJobs() {
    if (cache && cache.expiresAt > Date.now())
        return { jobs: cache.jobs, sources: cache.sources };
    ...
    if (!jobs.length)
        throw new Error('No configured public job board could be reached.');
