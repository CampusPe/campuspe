...
export async function createTailoredResumeDocx(text, outputPath) {
    const lines = cleanLines(text);
    if (!lines.length)
        throw new Error('The tailored resume has no content.');
    const children = [];
...
export function tailoredResumePreviewHtml(text) {
    const lines = cleanLines(text);
    const firstSectionIndex = lines.findIndex((item) => sectionNames.test(item.replace(/:$/, '')));
    const content = lines.map((line, index) => {
        ...
        if (index === 0)
            return `<h1>${safe}</h1>`;
        if (index > 0 && (firstSectionIndex < 0 || index < firstSectionIndex))
            return `<p class="contact">${safe}</p>`;
        if (sectionNames.test(line.replace(/:$/, '')))
            return `<h2>${escapeHtml(line.replace(/:$/, '').toUpperCase())}</h2>`;
        if (bulletPattern.test(line))
            return `<p class="bullet"><span>•</span>${escapeHtml(line.replace(bulletPattern, ''))}</p>`;
