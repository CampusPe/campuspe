...
test('prevents AI inference for factual and preference answers', () => {
    for (const field of ['work_authorized', 'us_sponsorship', 'active_immigration_case', 'current_salary', 'expected_salary', 'willing_in_office', 'willing_relocate'])
        assert.ok(nonInferableFields.has(field));
});
test('keeps voluntary demographic fields out of AI fallback', () => {
    for (const field of ['pronouns', 'gender', 'ethnicity', 'disability', 'veteran'])
        assert.ok(sensitiveFields.has(field));
});
...
test('keeps interview acknowledgements manual unless explicitly approved', () => {
    ...
    assert.equal(approvedDeclarationAnswer('i acknowledge that i do not have permission to record or transcribe any part of the interview process', ['Select...', 'I acknowledge']), 'I acknowledge');
