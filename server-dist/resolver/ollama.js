export function matchVisibleOption(answer, options) {
    if (!options.length)
        return answer;
...
export class OllamaAnswerProvider {
...
Candidate context: ${JSON.stringify(candidate).slice(0, 12_000)}`;
        try {
            ...
            const draft = JSON.parse(payload.message?.content || '{}');
            if (draft.needsUserInput)
                return null;
            if (!['string', 'number', 'boolean'].includes(typeof draft.answer))
                return null;
    ...
    plainText(value) { return value.replace(/<[^>]+>/g, ' ').replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim(); }
    limitWrittenAnswer(value) {
