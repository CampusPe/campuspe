...
export function classifyQuestion(normalized) {
...
export function questionSimilarity(left, right) {
    if (left === right)
        return 1;
    const tokens = (value) => new Set(value.split(' ').filter((token) => token.length > 2 && !['the', 'and', 'you', 'your', 'are', 'for', 'this', 'with'].includes(token)));
    const a = tokens(left);
