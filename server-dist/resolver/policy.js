...
export function approvedDeclarationAnswer(normalized, options = []) {
    ...
    if (!isNoRecordingAcknowledgement)
        return null;
    return options.find((option) => /^i acknowledge\.?$/i.test(option.trim())) ?? null;
}
export function manualPolicyReason(normalized) {
