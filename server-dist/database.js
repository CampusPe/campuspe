...
const automationRunColumns = new Set(db.prepare('PRAGMA table_info(automation_runs)').all().map((column) => column.name));
if (!automationRunColumns.has('auto_submit'))
    db.exec('ALTER TABLE automation_runs ADD COLUMN auto_submit INTEGER NOT NULL DEFAULT 0');
if (!automationRunColumns.has('test_mode'))
    db.exec('ALTER TABLE automation_runs ADD COLUMN test_mode INTEGER NOT NULL DEFAULT 0');
const optimizationColumns = new Set(db.prepare('PRAGMA table_info(resume_optimizations)').all().map((column) => column.name));
if (!optimizationColumns.has('storage_name'))
    db.exec('ALTER TABLE resume_optimizations ADD COLUMN storage_name TEXT');
