...
const plainText = (value) => value.replace(/<style[\s\S]*?<\/style>|<script[\s\S]*?<\/script>/gi, ' ').replace(/<[^>]+>/g, ' ').replace(/&nbsp;/gi, ' ').replace(/&amp;/gi, '&').replace(/&#39;/g, "'").replace(/&quot;/gi, '"').replace(/\s+/g, ' ').trim();
export async function extractTargetJob(jobUrl) {
    const canonicalUrl = canonicalJobUrl(jobUrl);
    const adapter = detectAdapter(canonicalUrl);
    if (!adapter)
        throw new Error('Use a supported Ashby, Greenhouse, Lever, Workable, Rippling, Breezy, BambooHR, or Recruitee job link.');
    ...
    try {
        ...
        if (!details.jobTitle || description.length < 100)
            throw new Error('The job requirements could not be read from this link. Try the public job-posting URL instead of an application-step URL.');
...
export async function extractResumeText(path) {
...
export async function reviewResume(text, job) {
    if (text.trim().length < 120)
        throw new Error('Very little text could be read from this resume. Upload a text-based PDF or DOCX file.');
...
${text.slice(0, 24_000)}`;
    try {
        ...
        if (!Number.isFinite(parsed.score) || !parsed.summary || !Array.isArray(parsed.strengths) || !Array.isArray(parsed.tips))
            throw new Error('Invalid review response');
        const baselineTips = fallbackReview(text).tips;
        const modelTips = parsed.tips.slice(0, 8).map((tip) => ({ priority: priorities.has(tip.priority) ? tip.priority : 'medium', title: String(tip.title).slice(0, 150), detail: String(tip.detail).slice(0, 700) }));
        const tips = [...modelTips];
        for (const tip of baselineTips)
            if (tips.length < 4 && !tips.some((item) => item.title.toLowerCase() === tip.title.toLowerCase()))
                tips.push(tip);
        return {
...
export async function optimizeResume(text, job, currentScore) {
    if (text.trim().length < 120)
        throw new Error('Very little text could be read from this resume.');
    const endpoint = process.env.OLLAMA_URL || 'http://127.0.0.1:11434';
    const model = process.env.OLLAMA_MODEL || 'gemma3:4b';
    const schema = {
...
ORIGINAL RESUME\n${text.slice(0, 24_000)}`;
    ...
    const payload = await response.json();
    const parsed = JSON.parse(payload.message?.content || '{}');
    if (!Array.isArray(parsed.changes) || !parsed.tailoredResumeText || !parsed.headline)
        throw new Error('The local AI did not return a usable tailored resume.');
    ...
    if (!changes.length && tailoredResumeText.trim() !== text.trim())
        changes.push({ section: 'Full resume structure and emphasis', before: text.slice(0, 2_000), after: tailoredResumeText.slice(0, 2_000), reason: 'The local AI returned a complete tailored draft without itemizing its edits. Review this side-by-side comparison carefully before approval.' });
    return {
