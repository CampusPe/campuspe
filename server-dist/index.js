...
app.post('/api/auth/login', (request, response, next) => {
    try {
        ...
        const user = db.prepare('SELECT id, email, name, password_hash, password_salt FROM users WHERE email = ?').get(input.email);
...
const getProfile = (userId) => db.prepare('SELECT users.name, users.email, profiles.* FROM profiles JOIN users ON users.id = profiles.user_id WHERE profiles.user_id = ?').get(userId);
...
app.post('/api/profile/resume/optimizations/:id/approve', requireAuth, async (request, response, next) => {
    try {
        ...
        const row = db.prepare('SELECT proposal_json FROM resume_optimizations WHERE id=? AND user_id=?').get(id, request.user.id);
        if (!row)
...
app.get('/api/profile/resume/optimizations/:id/download', requireAuth, (request, response, next) => {
    try {
        const { id } = resumeOptimizationIdSchema.parse(request.params);
        const row = db.prepare('SELECT job_title,company,status,storage_name FROM resume_optimizations WHERE id=? AND user_id=?').get(id, request.user.id);
        if (!row)
            return response.status(404).json({ error: { code: 'OPTIMIZATION_NOT_FOUND', message: 'Tailored resume draft not found.' } });
...
app.get('/api/jobs/saved', requireAuth, (request, response) => {
    const rows = db.prepare('SELECT job_json, saved_at FROM saved_jobs WHERE user_id=? ORDER BY saved_at DESC').all(request.user.id);
