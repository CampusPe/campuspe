...
function event(runId, status, message, detail) {
...
function update(runId, status, currentStep, values = {}) {
...
export class AutomationManager {
    create(userId, jobUrl, autoSubmit = false, testMode = false) {
    ...
    get(userId, runId) {
        const row = db.prepare('SELECT * FROM automation_runs WHERE id=? AND user_id=?').get(runId, userId);
        if (!row)
            return null;
        const events = db.prepare('SELECT status,message,detail_json,created_at FROM automation_events WHERE run_id=? ORDER BY created_at').all(runId);
    ...
    list(userId) { return db.prepare('SELECT * FROM automation_runs WHERE user_id=? ORDER BY created_at DESC LIMIT 200').all(userId).map(serializeRun); }
    pauseRun(userId, runId) {
        const row = db.prepare('SELECT * FROM automation_runs WHERE id=? AND user_id=?').get(runId, userId);
        if (!row)
    ...
    async process(runId, userId) {
        ...
        try {
            ...
            if (!active) {
                ...
                event(runId, 'OPENING_JOB', 'Opening a visible Chrome window.');
                ...
                if (!persistent?.browser.isConnected()) {
                    ...
                    browser.on('disconnected', () => {
                        ...
                        for (const [activeRunId, activeRun] of activeRuns) {
                            ...
                            const current = db.prepare('SELECT status FROM automation_runs WHERE id=?').get(activeRunId);
            ...
            if (stopForManualPause())
                return;
            update(runId, 'EXTRACTING_JOB', 'WAITING_FOR_FORM');
            event(runId, 'EXTRACTING_JOB', `Waiting for the ${this.boardName(adapter.id)} application form.`);
            ...
            const savedResume = db.prepare('SELECT resume_storage_name,resume_filename,resume_mime FROM profiles WHERE user_id=?').get(userId);
            const tailored = db.prepare("SELECT storage_name,job_title FROM resume_optimizations WHERE user_id=? AND job_url=? AND status='APPROVED' AND storage_name IS NOT NULL ORDER BY approved_at DESC LIMIT 1").get(userId, row.job_url);
            ...
            const profileEducation = db.prepare('SELECT education_json FROM profiles WHERE user_id=?').get(userId);
            ...
            for (let questionIndex = 0; questionIndex < questions.length; questionIndex += 1) {
                ...
                if (question.inputType === 'file' && question.id === '_systemfield_resume') {
                    ...
                    if (question.required)
                        return this.pause(runId, question, 'A saved resume is required for this application.');
                    continue;
                }
                ...
                if (question.required)
                    return this.pause(runId, question, resolution.reason);
            ...
            const failedState = db.prepare('SELECT current_step,job_board FROM automation_runs WHERE id=?').get(runId);
            ...
            update(runId, 'FAILED', 'FAILED', { error: message });
    ...
    pause(runId, question, reason) {
        const pause = { question, reason, instruction: 'Complete this field in the visible browser, then click Continue automation.' };
        update(runId, 'PAUSED_NEEDS_INPUT', 'WAITING_FOR_USER', { pause });
        event(runId, 'PAUSED_NEEDS_INPUT', `Waiting for “${question.text}”.`, pause);
    }
    pauseForBlocker(runId, status, message, currentStep = 'WAITING_FOR_USER') {
        const pause = { reason: message, instruction: 'Complete the step in the visible browser, then click Continue automation.' };
        update(runId, status, currentStep, { pause });
        event(runId, status, message);
    }
