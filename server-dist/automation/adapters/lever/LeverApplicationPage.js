...
export function leverOptionMatches(candidate, desired) {
...
export class LeverApplicationPage {
    ...
    async fill(question, answer) {
        ...
        if (/^current location$/i.test(question.text)) {
            ...
            await input.click();
            await input.fill('');
            await input.pressSequentially(desired, { delay: this.random(75, 125) });
            await this.pause(850, 1_350);
            const result = this.page.locator(`${leverSelectors.locationResults} > *`).first();
            if (!await result.isVisible().catch(() => false))
                throw new Error(`Lever did not suggest a location for “${desired}”.`);
            await result.click();
        }
            ...
            if (!option)
                throw new Error(`No matching Lever choice for “${desired}”.`);
            await option.hover();
            ...
            if (!option)
                throw new Error(`No matching Lever option for “${desired}”.`);
            await input.hover();
            ...
            await input.click();
            await input.fill('');
            await input.pressSequentially(desired, { delay: this.random(75, 125), timeout: 90_000 });
        }
        await this.pause(700, 1_200);
    }
    async fillEducation(_education) { throw new Error('Lever education fields require per-form handling.'); }
    async uploadResume(resume) { await this.page.locator(leverSelectors.resume).setInputFiles(resume); }
    async waitForResumeParsing() {
        ...
        while (Date.now() < deadline) {
            if (await failure.isVisible().catch(() => false))
                throw new Error('Lever could not parse the résumé. Check the uploaded file in the visible browser.');
            ...
            if (!parsingVisible && (successVisible || filenameVisible))
                return;
    ...
    async submit() {
        ...
        if (!await button.count())
            throw new Error('The Lever Submit Application button is not available.');
        ...
        if (await button.isVisible().catch(() => false))
            throw new Error('Lever did not confirm submission. Review the highlighted fields in the browser.');
    }
    inputFor(question) {
        const value = question.locator.value;
        if (value === 'id:resume-upload-input')
            return this.page.locator(leverSelectors.resume).first();
    ...
    async matchOption(options, answer) {
        const desired = this.normalize(answer);
        let partial = null;
        for (let index = 0; index < await options.count(); index += 1) {
            const option = options.nth(index);
            const candidate = this.normalize(await option.getAttribute('value') || '');
            if (candidate === desired)
                return option;
            if (!partial && leverOptionMatches(candidate, desired))
                partial = option;
        }
        return partial;
    }
    async matchSelectOption(select, answer) {
        const desired = this.normalize(answer);
        return select.locator('option').evaluateAll((options, normalized) => {
            ...
            for (const option of options) {
                ...
                if (candidate === normalized)
                    return option.value;
                if (!partial && (candidate.includes(normalized) || normalized.includes(candidate)))
