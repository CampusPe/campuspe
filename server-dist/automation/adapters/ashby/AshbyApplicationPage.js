...
export function ashbyChoiceMatches(label, desired) {
    if (label === desired)
        return true;
...
export function ashbyRadioChoiceMatches(label, desired) {
    return label === desired;
}
export function ashbyChoiceAnswerParts(answer, inputType) {
...
export class AshbyApplicationPage {
    ...
    async readQuestions() {
        return this.page.locator(ashbySelectors.fieldContainer).evaluateAll((containers) => {
            ...
            for (const [index, container] of containers.entries()) {
                const directLabel = [...container.children].find((child) => child.matches('label.ashby-application-form-question-title'));
                const label = directLabel ?? container.querySelector('label.ashby-application-form-question-title');
                ...
                let fieldType = input instanceof HTMLTextAreaElement ? 'textarea' : yesNo ? 'boolean' : choices.length ? 'select' : input instanceof HTMLSelectElement || input?.getAttribute('role') === 'combobox' ? 'select' : inputType === 'number' ? 'number' : 'text';
                if (inputType === 'file')
                    fieldType = 'text';
            ...
            if (education) {
                ...
                if (existing) {
                    existing.id = '_systemfield_education_history';
                    existing.fieldType = 'text';
                    existing.inputType = 'education';
                    existing.locator = { kind: 'education', value: '_systemfield_education_history' };
                    existing.answered = Boolean(school?.value.trim());
                }
                else
                    questions.push({ id: '_systemfield_education_history', text: 'Education History', fieldType: 'text', required: true, locator: { kind: 'education', value: '_systemfield_education_history' }, answered: Boolean(school?.value.trim()), inputType: 'education' });
            }
            return questions;
        });
    }
    ...
    async fill(question, answer) {
        ...
        if (await yesNo.count()) {
            ...
            await option.hover();
            await this.pause(500, 950);
            await option.click();
            await this.pause(650, 1_150);
            return;
        }
        ...
        if (await choices.count()) {
            ...
            for (let index = 0; index < await choices.count(); index += 1) {
                ...
                if (matches) {
                    ...
                    await this.pause(650, 1_150);
                    matched += 1;
                    if (question.inputType === 'radio-group')
                        break;
                }
            }
            if (matched)
        ...
        if (tag === 'select') {
            await input.hover();
            await this.pause(450, 850);
            await input.click();
            await this.pause(550, 1_000);
            ...
            if (question.fieldType === 'select') {
                ...
                if (!option)
                    throw new Error(`No matching dropdown option for “${String(answer)}”.`);
                await option.hover();
    ...
    async fillEducation(records) {
        const education = records.filter((record) => record.school.trim() && record.degree.trim());
        if (!education.length)
            throw new Error('Add a school and degree to your JobCopilot profile first.');
        for (const [index, record] of education.entries()) {
            ...
            await school.click();
            await school.fill('');
            await school.pressSequentially(record.school, { delay: this.random(75, 120), timeout: 90_000 });
            await this.pause(850, 1_350);
            ...
            if (!await schoolOption.count())
                throw new Error(`Choose the school “${record.school}” manually.`);
            await schoolOption.hover();
            await this.pause(500, 900);
            await schoolOption.click();
            await this.pause(700, 1_200);
            await this.typeNaturally(entry.locator('input[id$="-degree"]'), record.degree);
            if (record.field.trim())
                await this.typeNaturally(entry.locator('input[id$="-major"]'), record.field);
            const selects = entry.locator('select');
            const start = this.dateParts(record.startDate);
            const end = this.dateParts(record.endDate);
            if (start.month)
                await selects.nth(0).selectOption(start.month);
            if (start.year)
                await selects.nth(1).selectOption(start.year);
            if (end.month)
                await selects.nth(2).selectOption(end.month);
            if (end.year)
                await selects.nth(3).selectOption(end.year);
            if (record.current || !record.endDate.trim())
                await entry.locator('input[id$="-isCurrent"]').check();
            await this.pause(800, 1_400);
        }
    }
    async uploadResume(resume) { await this.page.locator(ashbySelectors.resume).setInputFiles(resume); }
    async uploadFile(question, file) {
        const container = await this.containerFor(question);
        const input = container.locator('input[type="file"]').first();
        if (!await input.count())
            throw new Error(`Could not locate the upload control for “${question.text}”.`);
    ...
    async submit() {
        ...
        if (!await button.isVisible())
            throw new Error('The Ashby submit button is not available.');
        ...
        if (await button.isVisible().catch(() => false))
            throw new Error('Ashby did not confirm submission. Review the highlighted fields in the browser.');
    }
    async containerFor(question) {
        ...
        if (await byPath.count())
            return byPath;
        const byId = this.page.locator(`#${safeId}`).first();
        if (await byId.count())
            return byId.locator('xpath=ancestor::*[contains(@class,"ashby-application-form-field-entry") or self::fieldset][1]').or(byId).first();
    ...
    async dropdownOption(answer, question) {
        ...
        for (let index = 0; index < await options.count(); index += 1) {
            ...
            if (candidate === desired || candidate.includes(desired) || desired.includes(candidate))
                return option;
            if (!/location|city/i.test(question))
                continue;
    ...
    async choiceLabel(container, choice) {
    ...
    async typeNaturally(input, value) {
        await input.hover();
        await this.pause(400, 750);
        await input.click();
        await this.pause(300, 600);
        await input.fill('');
        await input.pressSequentially(value, { delay: this.random(75, 120), timeout: 90_000 });
        await this.pause(700, 1_200);
    }
}
