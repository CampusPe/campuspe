...
export class AshbyJobPage {
    ...
    static supports(url) { return url.hostname.toLowerCase() === 'jobs.ashbyhq.com' && url.pathname.split('/').filter(Boolean).length >= 2; }
    ...
    async openApplication(jobUrl) {
        ...
        const targetIndex = await applyLinks.evaluateAll((links) => {
            ...
            links.forEach((link, index) => { const top = link.getBoundingClientRect().top + window.scrollY; if (top > greatestTop) {
                greatestTop = top;
                selected = index;
            } });
            return selected;
    ...
    async readDetails(originalUrl) {
        const embedded = await this.page.evaluate(() => window.__appData ?? null);
