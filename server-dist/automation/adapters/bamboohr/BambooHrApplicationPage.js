...
export class BambooHrApplicationPage {
    ...
    async readQuestions() {
        return this.page.locator(bambooHrSelectors.form).evaluate((form) => {
            ...
            for (const control of controls) {
                if (control.name?.startsWith('nickname_') || control.id === 'g-recaptcha-response' || control.name === 'g-recaptcha-response')
                    continue;
                const isResume = control.type === 'file';
            ...
            if (countryIndex > 0)
                questions.unshift(...questions.splice(countryIndex, 1));
            const stateIndex = questions.findIndex((question) => question.id === 'state.value');
            if (stateIndex >= 0 && stateIndex < questions.length - 1)
                questions.push(...questions.splice(stateIndex, 1));
            return questions;
        });
    }
    async focus(question) { await this.interactiveFor(question).scrollIntoViewIfNeeded(); await this.pause(350, 650); }
    async fill(question, answer) {
        const desired = String(answer);
        if (question.inputType === 'select') {
            const button = this.selectButton(question);
            await button.click();
            await this.pause(300, 550);
            const option = this.page.getByRole('option', { name: new RegExp(`^${this.escapeRegex(desired)}$`, 'i') }).first();
            if (!await option.isVisible().catch(() => false))
                throw new Error(`No matching BambooHR option for “${desired}”.`);
            await option.click();
        }
            ...
            if (!await radio.count())
                throw new Error(`No matching BambooHR choice for “${desired}”.`);
    ...
    async fillEducation(_education) { throw new Error('BambooHR education fields require per-form handling.'); }
    async uploadResume(resume) { await this.page.locator(bambooHrSelectors.resume).setInputFiles(resume); }
    async waitForResumeParsing() { const resumeId = this.page.locator(bambooHrSelectors.resumeId); const deadline = Date.now() + 30_000; while (Date.now() < deadline) {
        if ((await resumeId.inputValue().catch(() => '')).trim())
            return;
        await this.page.waitForTimeout(300);
    } throw new Error('BambooHR did not finish saving the résumé. Check the uploaded file in the visible browser.'); }
    async uploadFile(question, file) { await this.inputFor(question).setInputFiles(file); }
    async detectBlocker() { const captcha = this.page.locator(bambooHrSelectors.captchaChallenge).first(); return await captcha.isVisible().catch(() => false) ? { type: 'CAPTCHA', message: 'Complete the CAPTCHA in the visible browser, then continue.' } : null; }
    async isReadyForReview() { return this.page.locator(bambooHrSelectors.submit).isVisible().catch(() => false); }
    async submit() { const button = this.page.locator(bambooHrSelectors.submit); if (!await button.isVisible())
        throw new Error('The BambooHR Submit Application button is unavailable.'); await button.click(); const confirmation = this.page.getByText(/application (?:submitted|received)|thank you for applying/i).first(); await confirmation.waitFor({ state: 'visible', timeout: 20_000 }).catch(() => undefined); const blocker = await this.detectBlocker(); if (blocker)
        throw new Error(blocker.message); if (await button.isVisible().catch(() => false))
        throw new Error('BambooHR did not confirm submission. Review the highlighted fields in the browser.'); }
    inputFor(question) { const value = question.locator.value; if (value === '_systemfield_resume')
        return this.page.locator(bambooHrSelectors.resume); if (value.startsWith('name:'))
        return this.page.locator(`[name="${this.escapeAttribute(value.slice(5))}"]`).first(); if (value.startsWith('id:'))
        return this.page.locator(`#${this.escapeId(value.slice(3))}`).first(); throw new Error(`Could not locate BambooHR field “${question.text}”.`); }
    interactiveFor(question) { return question.inputType === 'select' ? this.selectButton(question) : this.inputFor(question); }
    selectButton(question) { return this.inputFor(question).locator('xpath=ancestor::*[@data-fabric-component="SelectField InputWrapper"][1]//button[@aria-haspopup="true"]').first(); }
