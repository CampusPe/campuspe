...
export function isGreenhouseSchoolSearch(text) {
...
export class GreenhouseApplicationPage {
    ...
    async readQuestions() {
        return this.page.locator(`${greenhouseSelectors.form} input[id], ${greenhouseSelectors.form} textarea[id], ${greenhouseSelectors.form} select[id]`).evaluateAll((controls) => {
            ...
            for (const control of controls) {
                const input = control;
                if (!input.id || seen.has(input.id) || input.type === 'hidden' || input.name === 'g-recaptcha-response' || input.id.startsWith('iti-'))
                    continue;
                seen.add(input.id);
                ...
                const fieldType = inputType === 'textarea' ? 'textarea' : inputType === 'select' ? 'select' : inputType === 'number' ? 'number' : 'text';
    ...
    async extractOptions(question) {
        if (isGreenhouseSchoolSearch(question.text))
            return [];
        const input = this.inputFor(question);
        const tag = await input.evaluate((element) => element.tagName.toLowerCase());
        if (tag === 'select')
            return input.locator('option:not([disabled])').evaluateAll((options) => options.map((option) => option.textContent?.trim() || '').filter(Boolean));
        await input.scrollIntoViewIfNeeded();
        await input.click();
    ...
    async fill(question, answer) {
        ...
        if (tag === 'select') {
            ...
            if (schoolField && !option) {
                ...
                if (await visibleOther.isVisible().catch(() => false))
                    option = visibleOther;
            }
            if (schoolField && !option) {
            ...
            if (!option)
                throw new Error(schoolField ? `Greenhouse did not offer “${desiredAnswer}” or an “Other” school option.` : `No matching Greenhouse option for “${desiredAnswer}”.`);
            await option.hover();
            await this.pause(350, 650);
            await option.click();
            if (schoolField) {
    ...
    async fillEducation(_education) { throw new Error('Greenhouse education fields require per-form handling.'); }
    async uploadResume(resume) { await this.page.locator(greenhouseSelectors.resume).setInputFiles(resume); }
    async uploadFile(question, file) {
        const input = this.inputFor(question);
        if (await input.getAttribute('type') !== 'file')
            throw new Error(`Could not locate the Greenhouse upload control for “${question.text}”.`);
    ...
    async submit() {
        ...
        if (!await button.isVisible())
            throw new Error('The Greenhouse submit button is not available.');
        await button.scrollIntoViewIfNeeded();
        await this.pause(650, 1_100);
        await button.hover();
        await this.pause(450, 800);
        await button.click();
        ...
        if (await button.isVisible().catch(() => false))
            throw new Error('Greenhouse did not confirm submission. Review the highlighted fields in the browser.');
    }
    inputFor(question) { return this.page.locator(`#${this.escapeId(question.locator.value)}`).first(); }
    escapeId(value) { return value.replace(/([^a-zA-Z0-9_-])/g, '\\$1'); }
    async dropdownOption(answer, allowCandidateSubset = false, allowRangeMatch = false) {
        ...
        for (let index = 0; index < await options.count(); index += 1) {
            ...
            if (candidate === desired || candidate.startsWith(`${desired} `))
                return option;
            const candidateTokens = candidate.split(' ');
            const desiredTokens = desired.split(' ');
            if (desiredYears !== null && this.rangeContains(await option.innerText(), desiredYears))
                return option;
            if (allowCandidateSubset && candidateTokens.length >= 2 && candidateTokens.every((token) => desiredTokens.includes(token)))
                return option;
            if (!partial && desiredTokens.every((token) => candidateTokens.includes(token)))
                partial = option;
        }
        return partial;
    }
    representativeYears(value) {
        const normalized = value.toLowerCase().replace(/[–—]/g, '-');
        const numbers = [...normalized.matchAll(/\d+(?:\.\d+)?/g)].map((match) => Number(match[0]));
        if (!numbers.length)
            return null;
        if (/less than|under|below/.test(normalized))
            return Math.max(0, numbers[0] - 0.5);
        if (numbers.length >= 2)
            return (numbers[0] + numbers[1]) / 2;
        return numbers[0];
    }
    rangeContains(option, years) {
        ...
        if (/\+|or more|and above|at least/.test(normalized))
            return years >= numbers[0];
        if (numbers.length >= 2)
            return years >= numbers[0] && years <= numbers[1];
        return years === numbers[0];
    }
    async schoolOption(school) {
        const desired = this.normalize(school);
        const options = this.page.getByRole('option');
        let other = null;
        for (let index = 0; index < await options.count(); index += 1) {
            const option = options.nth(index);
