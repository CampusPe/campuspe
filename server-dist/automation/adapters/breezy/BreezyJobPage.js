...
export class BreezyJobPage {
    ...
    async readDetails(originalUrl) {
        const details = await this.page.evaluate(async () => {
            ...
            source.pathname = source.pathname.replace(/\/apply\/?$/i, '');
            source.search = '';
            source.hash = '';
            let description = '';
            let locationText = '';
            ...
            try {
                ...
                const meta = [...documentCopy.querySelectorAll('.details li, .position-details li')].map((node) => node.textContent?.replace(/\s+/g, ' ').trim()).filter(Boolean);
                locationText = meta[0] || '';
        ...
        const slug = url.pathname.match(/^\/p\/([^/]+)/i)?.[1] || null;
        return {
