...
export class BreezyApplicationPage {
    ...
    async readQuestions() {
        return this.page.locator(breezySelectors.form).evaluate((form) => {
            ...
            for (const control of controls) {
                if (control.closest('[aria-hidden="true"]') || control.getAttribute('tabindex') === '-1' || control.getAttribute('ng-model')?.includes('candidateSchool'))
                    continue;
                ...
                if (isResume)
                    text = 'Resume/CV';
                if (control.name === 'cSummary')
                    text = 'Experience Summary';
                if (control.name === 'cCoverLetter')
                    text = 'Cover Letter';
                if (control.name === 'gdprAgreement')
                    text = 'Recruitment Privacy Notice consent';
                if (!text && control instanceof HTMLSelectElement && control.matches('.salary-details'))
                    text = 'Desired Salary period';
                if (!text) {
                ...
                const fieldType = inputType === 'textarea' ? 'textarea' : inputType === 'select' || inputType === 'checkbox' ? 'select' : inputType === 'number' ? 'number' : 'text';
            ...
            const educationRequired = form.querySelector('#education_required')?.value === 'required';
            if (educationRequired)
                questions.push({ id: '_breezy_education', text: 'Education', fieldType: 'text', required: true, locator: { kind: 'education', value: '_breezy_education' }, answered: Boolean(form.querySelector('li[ng-repeat*="candidateSchool"]')), inputType: 'group' });
            return questions;
        });
    }
    async focus(question) {
        if (question.locator.kind === 'education') {
            await this.page.locator(breezySelectors.addEducation).scrollIntoViewIfNeeded();
            return;
        }
        const input = this.inputFor(question);
        if (question.inputType === 'file')
            await this.page.locator(breezySelectors.resumeButton).first().scrollIntoViewIfNeeded();
        else
    ...
    async fill(question, answer) {
        ...
        if (question.inputType === 'checkbox') {
            ...
            if (await input.isChecked() !== shouldCheck)
                await input.click();
        }
            ...
            await input.click();
            await input.fill('');
            await input.pressSequentially(desired, { delay: this.random(50, 90), timeout: 90_000 });
        }
        await this.pause(450, 800);
    }
    async fillEducation(education) {
        if (!education.length)
            throw new Error('Add education to your profile before completing this Breezy application.');
        for (let index = 0; index < education.length; index += 1) {
            if (index >= await this.page.locator(breezySelectors.educationEntry).count()) {
                await this.page.locator(breezySelectors.addEducation).click();
                await this.pause(350, 650);
            }
            const entry = this.page.locator(breezySelectors.educationEntry).nth(index);
            ...
            if (record.startDate)
                await entry.locator('[ng-model="candidateSchool.date_start"]').fill(this.dateValue(record.startDate));
            if (record.endDate && !record.current)
                await entry.locator('[ng-model="candidateSchool.date_end"]').fill(this.dateValue(record.endDate));
        }
    }
    async uploadResume(resume) { await this.page.locator(breezySelectors.resume).setInputFiles(resume); await this.pause(900, 1_300); }
    ...
    async submit() {
        ...
        if (!await button.isVisible())
            throw new Error('The Breezy Submit Application button is not available.');
        await button.scrollIntoViewIfNeeded();
        await this.pause(550, 900);
        await button.click();
        ...
        if (await button.isVisible().catch(() => false))
            throw new Error('Breezy did not confirm submission. Review the highlighted fields in the browser.');
    }
    inputFor(question) {
        const value = question.locator.value;
        if (value === 'id:main-attachment')
            return this.page.locator(breezySelectors.resume);
        if (value.startsWith('id:'))
            return this.page.locator(`#${this.escapeId(value.slice(3))}`).first();
        if (value.startsWith('name:'))
            return this.page.locator(`[name="${value.slice(5).replaceAll('"', '\\"')}"]`).first();
        if (value === 'class:salary-details')
            return this.page.locator('select.salary-details').first();
        throw new Error(`Could not locate Breezy field “${question.text}”.`);
    }
    dateValue(value) { return /^\d{4}-\d{2}-\d{2}$/.test(value) ? value : /^\d{4}-\d{2}$/.test(value) ? `${value}-01` : /^\d{4}$/.test(value) ? `${value}-01-01` : value.slice(0, 10); }
